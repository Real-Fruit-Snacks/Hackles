...
def get_ldap_channel_binding(bh: BloodHoundCE, domain: Optional[str] = None, severity: Severity = None) -> int:
    ...
    if results:
        vulnerable = sum(1 for r in results if r.get("ldap_signing") == False or r.get("channel_binding") == False)
        if vulnerable:
